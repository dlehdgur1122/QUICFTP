...
import (
	...
	quic "github.com/lucas-clemente/quic-go"//Added@
)
...
type ftpActiveSocket struct {
	conn   quic.Stream/**net.UDPConn*/
	session quic.Session//added
	host   string
	port   int
	logger Logger
}
...
func newActiveSocket(remote string, port int, logger Logger, sessionID string) (DataSocket, error) {
	...
	tlsConf := &tls.Config{
	    ...
	    NextProtos: []string{"QUIC_FILE_TRANSFER"},
	}

	logger.Print(sessionID, "Opening active data connection to "+connectTo)
    /*
	raddr, err := net.ResolveUDPAddr("udp", connectTo)
    ...
    */
    ...
    quicConn, err := quic.DialAddr(connectTo, tlsConf, nil)//added@
    if err != nil {
        fmt.Println(err)
    	return nil, err
    }
    quicStream, err := quicConn.OpenStreamSync(context.Background())//added@
    if err != nil {
        fmt.Println(err)
        return nil, err
    }
	...
	socket.conn = quicStream//tcpConn@
	socket.session = quicConn
	socket.host = remote
	socket.port = port
...
func (socket *ftpActiveSocket) quicClose() error {
	return socket.session.Close()
}
...
func (socket *ftpPassiveSocket) quicClose() error {
	socket.lock.Lock()
	defer socket.lock.Unlock()
	if socket.conn != nil {
		return socket.conn.Close()
...
func (socket *ftpPassiveSocket) Close() error {
	...
	if socket.udpConn != nil {
		return socket.udpConn.Close()
	}
	return nil
}

func (socket *ftpPassiveSocket) GoListenAndServe(sessionID string) (err error) {
	laddr, err := net.ResolveUDPAddr("udp4", net.JoinHostPort("", strconv.Itoa(socket.port)))//net.ResolveTCPAddr("tcp",)
	if err != nil {
	...
	udplistener, err = net.ListenUDP("udp4", laddr)//tcplistener@
	if err != nil {
		socket.logger.Print(sessionID, err)
		return
	}
    socket.udpConn = udplistener
	...
	err = udplistener.SetDeadline(time.Now().Add(acceptTimeout))//tcplistener@
	if err != nil {
		socket.logger.Print(sessionID, err)
		return
	}*///remove acceptTimeout for transfering big file
    listener, err := quic.Listen(udplistener, socket.tlsConfig, nil)
    if err != nil {
	...
	go func() {
		...
		session, err := listener.Accept(context.Background())//conn.err:=listener.Accept()@
		if err != nil {
			socket.err = err
			socket.logger.Print(sessionID,err)
			fmt.Println(err.Error()+" quic Session")
			return
		}
		...
		if err != nil{
		    socket.err = err
		    socket.logger.Print(sessionID,err)
		    fmt.Println(err.Error()+" quic Stream")
		    return
		}//added@
