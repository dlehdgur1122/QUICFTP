...
func simpleTLSConfig(certFile, keyFile string) (*tls.Config, error) {
	...
	if config.NextProtos == nil {
		config.NextProtos = []string{"QUIC_FILE_TRANSFER"}
	}
...
func (server *Server) ListenAndServe() error {
	...
	if server.ServerOpts.TLS {
		...
		if err != nil {
			return err
		}//use this when you have your own CertFile and keyfile
		//server.tlsConfig = generateTLSConfig()//added@
		curFeats += " AUTH TLS\n PBSZ\n PROT\n"
...
func (server *Server) Serve(l net.Listener) error {
	...
	for {
		...
		if err != nil {
			...
			return err
			server.logger.Printf("Error1: ", err.Error())
