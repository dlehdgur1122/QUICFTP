...
func (conn *Conn) passiveListenIP() string {
	...
	if len(conn.PublicIp()) > 0 {
		...
		listenIP = conn.conn.LocalAddr().(*net.TCPAddr).String()
	}
...
func (conn *Conn) Serve() {
	conn.logger.Print(conn.sessionID, "Sessionection Established")
	// send welcome
	...
	conn.Close()
	conn.logger.Print(conn.sessionID, "Sessionection Terminated")
}
...
func (conn *Conn) sendOutofbandData(data []byte) {
	...
	message := "Closing data connection, sent " + strconv.Itoa(bytes) + " bytes (" + fmt.Sprintf("%.3f",(float64(bytes)/(time.Since(start).Seconds()))/1000000)+"MB/s)"//testing with 100MB txt file
	conn.logger.Print(conn.sessionID, message)//writeMessage makes a problem
}
...
func (conn *Conn) sendOutofBandDataWriter(data io.ReadCloser) error {
	...
	message := "Closing data connection, sent " +strconv.Itoa(int(bytes)) + " bytes (" + fmt.Sprintf("%.3f",(float64(bytes)/(time.Since(start).Seconds()))/1000000)+"MB/s)"//testing with 100MB txt file
	conn.logger.Print(conn.sessionID, message)//writeMessage makes a problem
	conn.dataConn.quicClose()
	//conn.dataConn = nil
